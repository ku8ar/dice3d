...
const Btn = ({onClick, children, backgroundColor = 'transparent'}) => {
  return (
    <button style={{ color: 'white', border: '1px solid white', backgroundColor, width: 'auto', height: '2.5rem', maxHeight: '2.5rem', fontSize: '1.3rem', flex: '1' }} onClick={onClick}>
      {children}
    </button>
  )
}
...
const CheatBtn = ({ number, index, editRow, isSelected }) => {
  const onClick = useCallback(() => editRow(index, number), [editRow, index, number])
  return (
    <Btn onClick={onClick} backgroundColor={isSelected ? 'blue' : 'transparent'}>
      {number || 'rand.'}
    </Btn>
  )
}
...
const CheatRow = ({ editRow, removeRow, index, num }) => {
  const onRemoveClick = useCallback(() => removeRow(index), [removeRow, index])
  return (
    <div style={{display: 'flex'}}>
      ...
      <Btn index={index} onClick={onRemoveClick} backgroundColor='red'>-</Btn>
    </div>
  )
}
...
const Cheats = ({ cheatList, setCheat }) => {
  ...
  const editRow = useCallback((editedKey, newNum) => setCheat(cheatList.map((num, key) => key === editedKey ? newNum : num)), [cheatList])
  const removeRow = useCallback((removedKey) => setCheat(cheatList.filter((_, key) => key !== removedKey)), [cheatList])
  const addRow = useCallback(() => setCheat([...cheatList, 0]), [cheatList])
    ...
    <>
      <div style={{ fontFamily, color: '#d8d8d8', position: 'absolute', bottom: '0', height: '100px', width: '100%', backgroundColor: 'rgba(245, 245, 245, 0.3)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-around' }}>
          ...
          <button onClick={setOpen} style={{ fontSize: '40px' }}>
            ➡️
          </button>
        </div>
      </div>
      {isOpen && (
        <div style={{ position: 'absolute', top: 0, bottom: 0, left: 0, right: 0, backgroundColor: '#008000', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
          ...
          <div style={{flex: '1'}}>
            {cheatList.map((num, key) => <CheatRow key={key} index={key} editRow={editRow} removeRow={removeRow} num={num} />)}
